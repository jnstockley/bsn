...
def initialize_usage() -> QuotaUsage:
    timezone = "America/Los_Angeles" # YouTube API quota resets at midnight Pacific Time
    ...
    with Session(engine) as s:
        stmt = select(QuotaUsage).where(QuotaUsage.window_start == start_time, QuotaUsage.window_end == end_time)
        ...
        policy: QuotaPolicy | None = s.execute(stmt).scalar_one_or_none()
        if not policy:
            logger.warning("Quota policy for YouTube not found when initializing usage. Call initialize_policy() first.")
