...
import time  # noqa: F401 – used indirectly via patch("auth.oauth_v2.time.*")
...
class TestRefreshCredential(TestCase):
    @patch("auth.oauth_v2._save_credential")
...
class TestFetchDeviceCode(TestCase):
    @patch("auth.oauth_v2.requests.post")
...
class TestPollForTokens(TestCase):
    @patch("auth.oauth_v2.time.sleep")
    @patch("auth.oauth_v2.requests.post")
    ...
    @patch("auth.oauth_v2.time.sleep")
    @patch("auth.oauth_v2.time.monotonic")
...
class TestFetchUserInfo(TestCase):
    @patch("auth.oauth_v2.requests.get")
...
class TestAuthenticateWithDeviceCode(TestCase):
    @patch("auth.oauth_v2._save_credential")
    @patch("auth.oauth_v2._fetch_user_info")
    @patch("auth.oauth_v2._poll_for_tokens")
...
class TestRevokeExpiredTokens(TestCase):
    ...
    @patch("auth.oauth_v2.refresh_credential")
    @patch("auth.oauth_v2._is_expired")
    @patch("auth.oauth_v2.Session")
...
class TestGetAuthenticatedYoutubeService(TestCase):
    @patch("auth.oauth_v2.build")
    @patch("auth.oauth_v2._is_expired")
    @patch("auth.oauth_v2._row_to_credentials")
    @patch("auth.oauth_v2._load_credential")
    ...
    @patch("auth.oauth_v2.authenticate_with_device_code")
    @patch("auth.oauth_v2._load_credential")
    ...
    def test_returns_none_when_build_raises(
        ...
        with patch("auth.oauth_v2.build", side_effect=Exception("build failed")):
            result = get_authenticated_youtube_service()
